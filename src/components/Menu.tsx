...
export function Menu({ appsScriptUrl, onNavigateSettings }: MenuProps) {
  ...
  const CATEGORIES = Array.from(new Set(menuItems.map((item) => item.category)))
    .filter((cat): cat is string => typeof cat === 'string' && cat.toLowerCase() !== 'tất cả' && cat.toLowerCase() !== 'yêu thích');
        ...
        <div className="flex gap-2">
          <div className="relative flex-grow group">
            ...
            <input
              ...
              className="w-full bg-stone-50 dark:bg-stone-900/50 border-none pl-10 pr-4 py-2.5 rounded-2xl shadow-sm dark:shadow-none font-bold text-sm placeholder:font-medium focus:ring-2 focus:ring-[#C9252C]/10 transition-all"
            />
            {searchQuery && (
              <button
        ...
        <div className="flex gap-2 overflow-x-auto pb-0.5 -mx-4 px-4 scroll-smooth scrollbar-hide">
          {displayCategories.map((category, index) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl whitespace-nowrap text-[12px] font-bold transition-all tap-active border ${
          ...
          >
            <div className="bg-stone-900 dark:bg-white text-white dark:text-black px-6 py-4 rounded-2xl shadow-2xl flex items-center justify-between border border-white/10 dark:border-black/10">
              <div className="flex items-center gap-3">
                <div className="w-6 h-6 bg-[#C9252C] rounded-full flex items-center justify-center">
        ...
        <div className="p-8 bg-white dark:bg-stone-900 border-t border-stone-50 dark:border-stone-800 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-2xl font-black text-[#C9252C]">{finalTotalPrice.toLocaleString()}đ</p>
            </div>
          </div>
          <button
            onClick={(e) => onAdd({
